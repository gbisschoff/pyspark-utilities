...
class NaturalCubicSpline(Transformer, HasInputCol, HasOutputCol, HasOrder, HasIntercept, HasKnots, DefaultParamsReadable,
    ...
    def _transform(self, dataset: DataFrame) -> DataFrame:
        ...
        def f(x):
            d = ((np.maximum(np.subtract(x, knots[:-1]), 0) ** order) - (
                        np.maximum(np.subtract(x, knots[-1]), 0) ** order)) / np.subtract(knots[-1], knots[:-1])
            return Vectors.dense(np.hstack((x ** np.arange(start, 2), d[:-1] - d[-1])).astype(float).tolist())
